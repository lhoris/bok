...
> 누가(Agent) 무엇을(Skill) 어떻게 수행하는가. 원칙: **소수 역할 + 재사용 절차**. BMAD 12+ 에이전트 과잉을 경계하고(`research/01-ai-framework/bmad-method.md`), 단순성을 우선한다(`research/01-ai-framework/agentic-workflow.md`).
>
...
> 근거: Skills의 progressive disclosure(L1 ~100토큰 메타만 상주, 필요 시 본문 로드, `research/01-ai-framework/claude-code-skills.md`). 절차를 Skill에 넣으면 **에이전트 수를 늘리지 않고** 능력을 확장할 수 있다 → 로스터를 5개로 유지.
...
## 0.1 Anthropic 패턴 매핑 (`research/01-ai-framework/agentic-workflow.md`, `/multi-agent.md`)
...
- **근거**: Multi-agent Lead — "전략 수립·메모리 기록·서브에이전트 스폰·종합", "분해 품질이 전부"(`research/01-ai-framework/multi-agent.md`).
...
- **근거**: "각 서브에이전트는 자족 태스크·출력형식·신선한 컨텍스트, 서로 모름"(`research/01-ai-framework/multi-agent.md`).
...
- **근거**: Evaluator–Optimizer(`research/01-ai-framework/agentic-workflow.md`) + Adversarial Review(`research/01-ai-framework/bmad-method.md`) + "합의보다 근거"(헌장).
...
> 각 Skill = `SKILL.md`(frontmatter: name·description ~100토큰) + 절차 본문 + 리소스(템플릿·스크립트). progressive disclosure로 로드. 이식 가능·vendor-neutral(`research/01-ai-framework/claude-code-skills.md`).

## 2.1 Discover Skills
| Skill | 절차 요지 | 근거 |
|-------|----------|------|
| `code-archaeology` | 의존성/제어흐름 복원, 변경 히트맵, 데이터모델·업무규칙 후보 추출 | `research/02-enterprise-onboarding/software-archaeology.md` |
| `human-externalization` | 인터뷰·Event Storming 가이드 생성·요약 → tacit KU (provenance.kind=human) | `research/03-knowledge-engineering/domain-modeling.md`, `/knowledge-management-and-engineering.md` |
| `kg-extraction` | 텍스트/코드에서 엔티티·관계 추출(Extract→Define) | `research/03-knowledge-engineering/knowledge-graph.md` |
...
> 근거: BMAD 모듈 + Spec Kit Extensions/Presets/Bundles(`research/01-ai-framework/bmad-method.md`, `/spec-kit.md`). 도메인/기술 특화는 Skill 팩으로 배포(예: `pack-mainframe-cobol`, `pack-billing-domain`, `pack-spring-boot`). 코어 5 에이전트는 불변, 능력만 확장. vendor-neutral.
