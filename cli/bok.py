...
STALE_DAYS = {"reference": "reference_days", "how-to": "reference_days",
              "explanation": "explanation_days", "glossary": "glossary_days"}


def set_fm_field(path: pathlib.Path, field: str, value: str) -> None:
    """Replace a single frontmatter field line, preserving the rest byte-for-byte."""
    text = path.read_text(encoding="utf-8")
    pre, fm, body = text.split("---", 2)
    fm = re.sub(rf"(?m)^{field}:.*$", f"{field}:    {value}", fm, count=1)
    path.write_text("---".join([pre, fm, body]), encoding="utf-8")


def ground_provenance(root: pathlib.Path, prov: list[dict]) -> tuple[list[str], list[str]]:
    """Return (failures, unverifiable). File-level grounding (design/04 A.4 / M3)."""
    failures, unverifiable = [], []
    for p in prov:
        kind, loc = p.get("kind"), str(p.get("locator", ""))
        pathpart = loc.split("#")[0]
        if kind in ("human", "external", "runtime") or loc.startswith("http") or "/" not in pathpart:
            unverifiable.append(loc)
        elif not (root / pathpart).exists():
            failures.append(loc)
    return failures, unverifiable


def _contradiction_ids(kus: list[KU]) -> set[str]:
    out = set()
    for k in kus:
        for rtype, target in k.relations():
            if rtype == "contradicts":
                out.add(k.id); out.add(target)
    return out


def _is_stale(cfg: dict, ku: KU) -> bool:
    if ku.meta.get("status") == "draft":
        return False
    lv = str(ku.meta.get("last_verified", ""))
    try:
    ...
    if d.year <= 1970:
        return False
    days = cfg.get("staleness", {}).get(STALE_DAYS.get(ku.meta.get("kind"), "reference_days"), 90)
    return (datetime.date.today() - d).days > days


def cmd_validate(root: pathlib.Path, scope: str | None, sign: str | None, owner: str | None) -> int:
    cfg, kus, errors = load_project(root)
    if errors:
        print("SCHEMA ERRORS (fix before validate):")
        for e in errors:
            print("  -", e)
        return 1
    if scope:
        kus = [k for k in kus if k.meta.get("context") == scope]
    by_id = {k.id: k for k in kus}
    contra = _contradiction_ids(kus)

    # ---- sign-off path (human-in-the-loop, design/03 §4 / design/04 A.1) ----
    if sign:
        ku = by_id.get(sign)
        if not ku:
            sys.exit(f"error: KU not found: {sign}")
        fails, _ = ground_provenance(root, ku.meta.get("provenance") or [])
        cur = conf_index(ku.meta.get("confidence", ""))
        if fails:
            print(f"REFUSED: {sign} has grounding failures: {fails}"); return 2
        if sign in contra:
            print(f"REFUSED: {sign} is in an unresolved contradiction (cap: corroborated)"); return 2
        if cur < conf_index("corroborated"):
            print(f"REFUSED: {sign} is '{ku.meta.get('confidence')}' — needs >= corroborated before sign-off"); return 2
        set_fm_field(ku.path, "confidence", "verified")
        set_fm_field(ku.path, "last_verified", datetime.date.today().isoformat())
        if owner:
            set_fm_field(ku.path, "owner", owner)
        print(f"SIGNED: {sign} -> verified (owner={owner or 'n/a'}, {datetime.date.today()})")
        return 0

    # ---- full validation pass (Evaluator, design/02 §3) ----
    transitions, gfails, stale_hits = [], [], []
    for k in kus:
        fails, unver = ground_provenance(root, k.meta.get("provenance") or [])
        cur_name = k.meta.get("confidence", "")
        cur = conf_index(cur_name)
        kinds = {p.get("kind") for p in (k.meta.get("provenance") or [])}
        new = cur_name
        if fails:
            gfails.append((k.id, fails))
            if cur > conf_index("unverified"):
                new = "unverified"  # demote: evidence not found
        elif _is_stale(cfg, k):
            stale_hits.append(k.id)
            new = CONFIDENCE_ORDER[max(0, cur - 1)]
        elif cur_name == "inferred" and len(kinds) >= 2:
            new = "corroborated"  # cross-support: 2+ distinct-kind provenance
        # contradiction cap
        if k.id in contra and conf_index(new) >= conf_index("verified"):
            new = "corroborated"
        if new != cur_name:
            set_fm_field(k.path, "confidence", new)
            transitions.append((k.id, cur_name, new))

    # report
    lines = [
        "<!-- GENERATED by `bok validate` -->",
        f"# Validation Report{f' — {scope}' if scope else ''}",
        f"\n_생성: {datetime.date.today()} · KUs: {len(kus)}_",
        "\n## Transitions",
    ]
    lines += [f"- {i}: {a} → **{b}**" for i, a, b in transitions] or ["- (변화 없음)"]
    lines += ["\n## Grounding failures (근거 파일 없음 → 강등)"]
    lines += [f"- {i}: {locs}" for i, locs in gfails] or ["- (없음)"]
    lines += ["\n## Staleness 강등"]
    lines += [f"- {i}" for i in stale_hits] or ["- (없음)"]
    lines += ["\n## Contradictions (검증 대기)"]
    lines += [f"- {i}" for i in sorted(contra)] or ["- (없음)"]
    unver_count = sum(1 for k in kus if conf_index(k.meta.get("confidence", "")) <= conf_index("inferred"))
    lines += [f"\n## 남은 미검증(<= inferred): {unver_count} / {len(kus)}"]
    (root / "bok" / "_system" / "validation-report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

    print(f"validated {len(kus)} KU(s): {len(transitions)} transition(s), "
          f"{len(gfails)} grounding failure(s), {len(stale_hits)} stale.")
    for i, a, b in transitions:
        print(f"  {a} -> {b}: {i}")
    for i, locs in gfails:
        print(f"  GROUNDING FAIL {i}: {locs}")
    return 2 if (gfails or contra) else 0
...
def cmd_assemble(root: pathlib.Path, scope: str, goal: str, need: str | None,
                 budget: int, depth: int) -> int:
    cfg, kus, errors = load_project(root)
    if errors:
        print("SCHEMA ERRORS (fix before assemble):")
        for e in errors:
            print("  -", e)
        return 1
    all_ids = {k.id for k in kus}
    pool = [k for k in kus if k.meta.get("context") == scope]
    id2ku = {k.id: k for k in kus}
    text = {k.id: k.path.read_text(encoding="utf-8") for k in kus}

    terms = [t for t in re.split(r"\W+", goal.lower()) if len(t) >= 2]

    def relevance(k: KU) -> int:
        hay = f"{k.meta.get('title','')} {_tldr(text[k.id])} {k.id}".lower()
        return sum(1 for t in terms if t in hay)

    # 1. Filter + 2. Seed (relevance; optional need/kind)
    seeds = sorted([k for k in pool if relevance(k) > 0], key=relevance, reverse=True)
    if need:
        seeds = [k for k in seeds if k.meta.get("kind") == need] or seeds
    seeds = seeds[:5]

    # 3. Expand along relations (depth-bounded)
    tier: dict[str, str] = {k.id: "L3" for k in seeds}
    frontier = list(seeds)
    for _ in range(depth):
        nxt = []
        for k in frontier:
            for _rt, tgt in k.relations():
                if tgt in id2ku and tgt not in tier:
                    tier[tgt] = "L2"; nxt.append(id2ku[tgt])
        frontier = nxt

    # 4. Rank (relevance x confidence) & Trim to budget
    def est_tokens(kid: str, t: str) -> int:
        body = text[kid]
        return (len(body) if t == "L3" else len(_tldr(body)) + 200) // 4

    ranked = sorted(
        tier.items(),
        key=lambda it: (it[1] != "L3", -relevance(id2ku[it[0]]),
                        -conf_index(id2ku[it[0]].meta.get("confidence", ""))),
    )
    units, total = [], 0
    for kid, t in ranked:
        e = est_tokens(kid, t)
        if total + e > budget and t == "L3":
            t, e = "L2", est_tokens(kid, "L2")  # degrade detail before dropping
        if total + e > budget:
            continue
        total += e
        units.append((kid, t, id2ku[kid].meta.get("confidence")))

    # 5. Emit — warnings (low confidence) + gaps (known-unknowns)
    warnings = [f"{kid}: confidence={c} (< corroborated) — 결정 전 확인 권장"
                for kid, _t, c in units if conf_index(c) < conf_index("corroborated")]
    gaps = []
    for kid, _t, _c in units:
        for _rt, tgt in id2ku[kid].relations():
            if tgt not in all_ids:
                gaps.append(f"dangling: {tgt} (referenced by {kid}) — 미발굴")
    cov_path = root / "bok" / "_system" / "coverage.yaml"
    if cov_path.exists():
        cov = yaml.safe_load(cov_path.read_text(encoding="utf-8")) or {}
        for a in cov.get("areas", []):
            if not a.get("kus"):
                gaps.append(f"coverage gap: {a['id']} ({a.get('criticality')}) — 미발굴")

    pack = {
        "context_pack": {
            "goal": goal, "need": need, "bounded_context": scope,
            "units": [{"id": k, "tier": t, "confidence": c} for k, t, c in units],
            "warnings": warnings or ["(none)"],
            "gaps": gaps or ["(none)"],
            "token_estimate": total,
        }
    }
    out = yaml.safe_dump(pack, allow_unicode=True, sort_keys=False)
    (root / "bok" / "_system" / "context-pack.yaml").write_text(
        "# GENERATED by `bok assemble` — ephemeral per-task Context Pack (design/01 B.4)\n" + out,
        encoding="utf-8",
    )
    print(out)
    print(f"# {len(units)} unit(s), ~{total} tokens, {len(gaps)} gap(s), {len(warnings)} warning(s)")
    return 0
...
def main(argv=None) -> int:
    ...
    pa = sub.add_parser("assemble", help="build a Context Pack for a task (with gaps)")
    pa.add_argument("path", nargs="?", default=".")
    pa.add_argument("--scope", required=True)
    pa.add_argument("--goal", required=True)
    pa.add_argument("--need", default=None)
    ...
    pv.add_argument("--scope", default=None)
    pv.add_argument("--sign", default=None, help="KU id to record owner sign-off (-> verified)")
    pv.add_argument("--owner", default=None)
    ...
    if args.cmd == "assemble":
        return cmd_assemble(root, args.scope, args.goal, args.need, args.budget, args.depth)
    if args.cmd == "compile":
        return cmd_compile(root)
    if args.cmd == "validate":
        return cmd_validate(root, args.scope, args.sign, args.owner)
    if args.cmd == "status":
