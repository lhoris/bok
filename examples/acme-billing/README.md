...
| Readiness Tier (D04 B.4) | R0 (data-model red로 Mapped 미달), 목표 R3 |
...
bok/_system/catalog.yaml        (GENERATED by `bok compile`)
bok/_system/coverage.yaml       (authored input; status는 `bok ready`가 계산)
bok/_system/readiness-report.md (GENERATED by `bok ready`)
```

> `_system/`의 생성물은 `cli/bok.py`로 실제 재생성된다:
> ```
...
5. `_system/readiness-report.md` — **Hard gate FAIL → Tier R0 → gap 목록 → 다음 나선.**
