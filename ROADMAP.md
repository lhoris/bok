...
- `bok init` / `bok compile`(catalog·graph·dangling 검사) / `bok-schema` 검증.
- `bok ready`의 coverage→신호등→hard gate→tier 계산(D04) 구현.
- **검증 질문**: coverage 영역↔KU 자동 매핑이 손 없이 얼마나 정확한가(D04 F-1).
